...
public class ChatUIChat extends JFrame {
    ...
    public ChatUIChat(String username, Socket socket) throws IOException { //登录后接管方法
        this(); //引用上述方法与代码，显示聊天界面
        this.username = username; //设置全局username变量为登录用户名
        this.socket = socket; //设置全局socket变量为连接管道
        setTitle(username + "的聊天窗口"); //设置聊天窗口标题
        new ClientReaderThread(socket, this).start(); //启动客户端监听子线程
        msgSync(); //同步消息记录
    }

    private void msgSync() throws IOException { //消息记录同步
        DataOutputStream dos = new DataOutputStream(socket.getOutputStream()); //创建数据输出流
        dos.writeInt(8); //告知服务端将要同步消息
        dos.writeUTF(username); //告知服务端所要同步的客户端用户名
